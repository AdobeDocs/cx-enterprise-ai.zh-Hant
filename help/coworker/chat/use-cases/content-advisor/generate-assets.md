---
title: 同事行銷資產產生
description: 瞭解如何使用同事聊天為社交頻道產生品牌內行銷資產，以節省時間和精力。
role: User
level: Beginner, Intermediate
doc-type: Feature Video
duration: 147
last-substantial-update: 2026-09-01T00:00:00.000Z
jira: KT-22502
product_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
feature_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
source-git-commit: f1ab460d5f582a98011034004d591f68f50df372
workflow-type: tm+mt
source-wordcount: '140'
ht-degree: 0%
---

# 透過同事聊天產生行銷資產

Co-worker可簡化為社群媒體和其他頻道建立品牌資產的程式。 您可以描述目標，而「同事」會處理實現目標的步驟，而不必手動管理每個工作。

例如，若要建立Instagram咖啡貼文，您可以提供簡短的主意，Co-worker會提供概念建議，並產生最佳化內容。 它可選取適合的影像、調整尺寸，以及設定平台的格式，以節省時間並確保品質。

Co-worker還提供進一步自訂的彈性。 您可以修改資產、建立變數或調整內容以符合不同的格式，例如Instagram內文。 此方法可讓您專注於目標，而由同事管理技術細節。

>[!VIDEO](https://video.tv.adobe.com/v/3502720/?captions=chi_hant&learn=on&enablevpops)